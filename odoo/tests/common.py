...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    def _get_view_fields(self, node, model):
        fnames = set(el.get('name') for el in node.xpath('.//field[not(ancestor::field[position() = 2])]'))
    ...
    def _o2m_set_edition_view(self, descr, node, level):
        ...
        edition = {
            ...
            # 'arch': etree.tostring(views[edition_view]),
        }
    ...
    def _process_fvg(self, model, fvg, level=2):
        ...
        * pre-processed onchanges list
        ...
        for f in fvg['tree'].xpath('.//field[not(ancestor::field[position() = 2])]'):
            fname = f.get('name')
            order.append(fname)
