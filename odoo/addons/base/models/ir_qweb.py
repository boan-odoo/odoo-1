...
class QWebException(Exception):
    ...
    def __str__(self):
        parts = [self.title]
        if self.__cause__ is not None and str(self.__cause__):
            parts.append(f"{self.__cause__.__class__.__name__}: {self.__cause__}")
        elif self.__context__ is not None and str(self.__context__):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _render(self, template, values=None, **options):
        ...
        if 'xml' in tools.config['dev_mode']:
            ir_qweb = ir_qweb.with_context(use_qweb_cache=False)
        elif 'use_qweb_cache' not in ir_qweb.env.context:
