...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_pdf_prepare_streams(self, data, res_ids=None):
        ...
        # In case of test environment without enough workers to perform calls to wkhtmltopdf, fallback to render_html.
        if (tools.config['test_enable'] or tools.config['test_file']) and not self._context.get('force_report_rendering'):
            return None
            ...
            for record in records:
                ...
                if self_sudo.attachment:
                    ...
                    if attachment and self_sudo.attachment_use:
                        ...
                        # Ensure the stream can be savec in Image.
        ...
        remaining_res_ids = [record for record, stream_data in collected_streams.items() if not stream_data['stream']]
        is_whtmltopdf_needed = not res_ids or remaining_res_ids
        ...
        if is_whtmltopdf_needed:
            ...
            self_sudo_ctx = self_sudo.with_context(**additional_context)
            html = self_sudo_ctx._render_qweb_html(remaining_res_ids, data=data)[0]

            bodies, html_ids, header, footer, specific_paperformat_args = self_sudo_ctx._prepare_html(html)
            ...
            if len(remaining_res_ids) == 1:
                # Only one record: append the whole PDF.
                collected_streams[remaining_res_ids[0]]['stream'] = pdf_content_stream
            else:
                ...
                assert len(outlines_pages) == len(remaining_res_ids)
                ...
                for i, num in enumerate(outlines_pages):
                    ...
                    collected_streams[remaining_res_ids[i]]['stream'] = stream
    ...
    def _render_qweb_pdf(self, res_ids=None, data=None):
        ...
        if self.attachment:
            ...
            if attachment_vals_list:
                ...
                try:
                    ...
                    _logger.info("Cannot save PDF report %r attachments", attachment_names)
                else:
        ...
        if len(streams_to_merge) == 1:
            ...
            pdf_merged_stream = self._merge_pdfs(streams_to_merge)
            pdf_content = pdf_merged_stream.getvalue()
            pdf_merged_stream.close()
