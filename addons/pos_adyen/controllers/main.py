    ...
    def notification(self):
        ...
        if payment_method:
            ...
            if data['SaleToPOIResponse'].get('DiagnosisResponse'):
                ...
                pos_session = self._get_pos_session(data['SaleToPOIResponse']['MessageHeader']['SaleID'])
                if pos_session:
                    message_value = [payment_method.id, payment_method.get_latest_adyen_status(data['SaleToPOIResponse']['MessageHeader']['SaleID'])]
                    pos_session.broadcast_pos_message('adyen-payment-status-received', message_value)
                else:
                    _logger.error("received a message for a terminal that doesn't belong in a pos session: %s", terminal_identifier)

        else:
            _logger.error('received a message for a terminal not registered in Odoo: %s', terminal_identifier)

    def _get_pos_session(self, sale_id: str):
        regex = re.compile(r'SESSION_ID: (\d*)')
