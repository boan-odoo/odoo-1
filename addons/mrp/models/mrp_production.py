...
class MrpProduction(models.Model):
    ...
    def _split_productions(self, amounts=False, cancel_remaning_qty=False):
        ...
        for production in self:
            ...
            for workorder in production.workorder_ids:
                initial_workorder_remaining_qty.append(max(initial_qty - workorder.qty_carried - workorder.qty_produced, 0))
            ...
            for index, workorder in enumerate(bo.workorder_ids):
                ...
                workorder.qty_carried = max(workorder.qty_production - remaining_qty, 0)
                if remaining_qty:
                    initial_workorder_remaining_qty[index % workorders_len] = max(remaining_qty - workorder.qty_produced, 0)
                    if not workorders_to_update or workorders_to_update[-1].production_id != workorder.production_id:
