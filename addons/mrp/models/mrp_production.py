...
class MrpProduction(models.Model):
    ...
    def _compute_picking_type_id(self):
        productions_per_company = defaultdict(lambda: self.env['mrp.production'])
        for production in self:
            productions_per_company[production.company_id] |= production
        for company, productions in productions_per_company.items():
            default_picking_type_id = False
            for production in productions:
                if production.picking_type_id and production.picking_type_id.company_id == company:
                    continue
                # TODO test if context take the priority
                if not default_picking_type_id:
                    default_picking_type_id = self._get_default_picking_type_id(company.id)
                production.picking_type_id = default_picking_type_id
    ...
    def _compute_locations(self):
        for production in self:
            if not production.picking_type_id.default_location_src_id or not production.picking_type_id.default_location_dest_id.id:
    ...
    def _compute_product_id(self):
        for production in self:
            if production.bom_id and (
                not production.product_id or
                production.bom_id.product_tmpl_id != production.product_tmpl_id or
                production.bom_id.product_id and production.bom_id.product_id != production.product_id
            ):
                production.product_id = production.bom_id.product_id or production.bom_id.product_tmpl_id.product_variant_id

    @api.depends('product_id', 'picking_type_id')
    def _compute_bom_id(self):
        for production in self:
            if not production.product_id and not production.bom_id:
            ...
            if not production.bom_id or production.bom_id.product_tmpl_id != production.product_tmpl_id or (production.bom_id.product_id and production.bom_id.product_id != production.product_id):
                bom = self.env['mrp.bom']._bom_find(production.product_id, picking_type=production.picking_type_id, company_id=production.company_id.id, bom_type='normal')[production.product_id]
                if bom:
    ...
    def _set_date_deadline(self):
        # # TODO before merge: fix this ! It causes issue with `test_check_update_qty_mto_chain`.
        # # The move created by procurement group (`customer_move` in the test) will then have a
        # # `date_deadline` once the manufacturing order is created, which will make the procurement
        # # group unable to merge this move with the next one (a neg. move with -5 qty. in the test).
        # # For now, I don't understand why it's not the case in master.
        # for production in self:
    ...
    def write(self, vals):
        ...
        if 'move_raw_ids' in vals and self.state not in ['draft', 'cancel', 'done']:
            ...
            if vals.get('location_src_id'):
                location_srouce = self.env['stock.location'].browse(vals.get('location_src_id'))
                warehouse_id = location_srouce.warehouse_id.id
    ...
    def _create_update_move_finished(self):
        ...
        """
        # keep manual entries
        list_move_finished = [Command.link(move.id) for move in self.move_finished_ids.filtered(
