...
class IrQWeb(models.AbstractModel):
    ...
    def _prepare_frontend_environment(self, values):
        ...
        values.update(dict(
            ...
            res_company=current_website.company_id.sudo(),
        ...
        if 'inherit_branding' not in irQweb.env.context:
            if editable:
    ...
    def _post_processing_att(self, tagName, atts):
        ...
        if self.env.context.get('inherit_branding') or self.env.context.get('edit_translations') or \
            self.env.context.get('debug') or (request and request.session.debug):
            return atts
