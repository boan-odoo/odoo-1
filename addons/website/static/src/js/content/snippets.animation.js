...
registry.mediaVideo = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        if (this.$target.children('iframe').length) {
            ...
            this.videoSrc = _.escape(this.iframe.src);
            this.iframeID = this.iframe.id || this.iframeID;
            ...
            this.videoSrc = _.escape(this.$target.data('oe-expression') || this.$target.data('src'));
            // Validate the src to only accept supported domains we can trust
            var m = this.videoSrc.match(/^(?:https?:)?\/\/([^/?#]+)/);
            if (!m) {
    ...
    */
    _autoStartVideo: function () {
       ...
       if (this.isMobileEnv && this.isYoutubeVideo && this.autoplay) {
