...
var PaymentSix = PaymentInterface.extend({
    ...
    init: function () {
        ...
        var terminal_ip = arguments[1].six_terminal_ip
        var instanced_payment_methods = _.filter(this.pos.payment_methods, function(payment_method) {
            return payment_method.use_payment_terminal === "six"
                && payment_method.six_terminal_ip === terminal_ip
                && payment_method.payment_terminal
        })
        if (instanced_payment_methods.length) {
            var payment_terminal = instanced_payment_methods[0].payment_terminal
