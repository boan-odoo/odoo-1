...
class PaymentTransaction(models.Model):
    ...
    def _send_payment_request(self):
        ...
        if self.acquirer_id.capture_manually and self.token_id.state == 'done':
            status = 'authorized'
        else:
            status = self.token_id.state
    ...
    def _process_notification_data(self, notification_data):
        ...
        if status == 'draft':
            ...
            self._set_error(
                "Test: " +
                _("You selected the following state on the demo checkout page: %s", status)
            )
