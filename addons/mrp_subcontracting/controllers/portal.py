...
class CustomerPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'production_count' in counters:
            ...
            values['production_count'] = request.env['stock.picking'].search_count([('partner_id', '=', partner.id), ('move_ids.is_subcontract', '=', True)])
        return values

    @http.route(['/my/productions', '/my/productions/page/<int:page>'], type='http', auth="user", website=True)
    def portal_my_productions(self, page=1, date_begin=None, date_end=None, sortby='date', filterby='all', **kw):
        partner = request.env.user.partner_id
        StockPicking = request.env['stock.picking']
        domain = [('partner_id', '=', partner.id), ('move_ids.is_subcontract', '=', True)]
        ...
        request.session['my_productions_history'] = pickings.ids[:100]
