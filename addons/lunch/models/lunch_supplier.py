...
class LunchSupplier(models.Model):
    ...
    available_today = fields.Boolean('This is True when if the supplier is available today',
                                     compute='_compute_available_dates', search='_search_available_today')
    available_nextday = fields.Boolean(compute='_compute_available_dates')
    ...
    def _compute_available_dates(self):
        ...
        for supplier in self:
            ...
            supplier.available_nextday = supplier._available_on_date(now + relativedelta(days=1))
    ...
    def action_send_orders(self):
        ...
        for supplier in self:
            if supplier.send_by == 'mail':
                ...
                orders |= supplier._get_current_orders()
        orders.action_send()
    ...
    def action_confirm_orders(self):
        orders = self.env['lunch.order']
        for supplier in self:
            orders |= supplier._get_current_orders(state='sent')
