...
class AccountMove(models.Model):
    ...
    def _get_pay_term_lines_for_invoice_render(self):
        ...
        if term_lines.filtered('credit' if self.is_inbound() else 'debit'):
            due_term_lines = term_lines.filtered('debit' if self.is_inbound() else 'credit').sorted(key='date_maturity')
            return [{'date': date, 'amount': amount} for date, amount in zip(
                due_term_lines.mapped('date_maturity') + [self.invoice_date_due],
                due_term_lines.mapped(lambda l: abs(l.amount_currency)) + [self.amount_total],
            )]
        else:
            return False
