...
class AccountMove(models.Model):
    ...
    def _compute_payments_widget_to_reconcile_info(self):
        for move in self:
            ...
            if move.is_inbound():
                domain.append(('amount_currency', '<', 0.0))
...
class AccountMoveLine(models.Model):
    ...
    def _create_reconciliation_partials(self):
        ...
        while True:
            ...
            # ==== Match both lines and compute the partial amounts ====
            ...
            if debit_line.currency_id == credit_line.currency_id == company_currency \
                ...
                # Multiple involved foreign currencies.
                recon_currency = company_currency
                debit_rate = get_accounting_rate(debit_line)
                credit_rate = get_accounting_rate(credit_line)
                recon_debit_amount = remaining_debit_amount
            ...
            # ==== Match both lines together and compute amounts ====
            ...
            if recon_currency == company_currency:
                ...
                if debit_rate:
                    partial_debit_amount_currency = debit_line.currency_id.round(
                        recon_debit_amount
                        * debit_rate
                        * min_recon_amount
                        / recon_debit_amount
                ...
                if credit_rate:
                    partial_credit_amount_currency = credit_line.currency_id.round(
                        recon_credit_amount
                        * credit_rate
                        * min_recon_amount
                        / recon_credit_amount
    ...
    def reconcile(self):
        ...
        def is_line_reconciled(line):
            ...
            # In normal cases, the journal item is always reconciled at this point because the exchange difference
            # journal entries generated by the partials already managed the difference between
            # 'amount_residual_currency' and 'amount_residual'.
        ...
        if all(is_line_reconciled(line) for line in involved_lines):
            ...
            if not self._context.get('no_exchange_difference'):
                ...
                # Exchange difference on full to fix either rounding issues, either partial reconciliation in older
                # code version or anything else not handled by the exchange difference on partials.
                exchange_lines_to_fix = self.env['account.move.line']
                amounts_list = []
