            ...
            base_tax_matching_base_amounts AS (
                ...
                */
                ...
                SELECT
                    ...
                    ROUND(
                        COALESCE(ABS(sub.total_tax_amount) * sub.cumulated_base_amount / ABS(NULLIF(sub.total_base_amount, 0.0)), 0.0),
                        sub.comp_curr_prec
                    )
                    - LAG(ROUND(
                        COALESCE(ABS(sub.total_tax_amount) * sub.cumulated_base_amount / ABS(NULLIF(sub.total_base_amount, 0.0)), 0.0),
                    ...
                    ROUND(
                        COALESCE(ABS(sub.total_tax_amount_currency) * sub.cumulated_base_amount_currency / ABS(NULLIF(sub.total_base_amount_currency, 0.0)), 0.0),
                        sub.curr_prec
                    )
                    - LAG(ROUND(
                        COALESCE(ABS(sub.total_tax_amount_currency) * sub.cumulated_base_amount_currency / ABS(NULLIF(sub.total_base_amount_currency, 0.0)), 0.0),
