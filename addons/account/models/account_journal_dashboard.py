...
class account_journal(models.Model):
    ...
    has_holes = fields.Boolean(compute='_compute_has_holes')

    def _query_has_holes(self):
        self.env.cr.execute("""
    ...
    def _compute_has_holes(self):
        has_holes = set(journal_id for journal_id, _prefix in self._query_has_holes())
        for journal in self:
            journal.has_holes = journal.id in has_holes
    ...
    def show_holes(self):
        has_holes = self._query_has_holes()
        return {
            'type': 'ir.actions.act_window',
            ...
            'domain': expression.OR(
                ...
                for journal_id, prefix in has_holes
            ),
            'context': {
