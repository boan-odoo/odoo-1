...
export const PosLoyaltyPaymentScreen = (PaymentScreen) =>
    class extends PaymentScreen {
        ...
        async _postPushOrderResolve(order, server_ids) {
            ...
            let clientLoyaltyCardId = client ? client.loyalty_card_id : 0;
            let couponData = Object.values(order.couponPointChanges).reduce((agg, pe) => {
                ...
                if (program.is_nominative && order.get_client()) {
                    agg[pe.coupon_id].partner_id = client.id;
                }
                if (program.id === loyaltyProgramId) {
                    if (!clientLoyaltyCardId) {
                        clientLoyaltyCardId = pe.coupon_id;
                    }
                }
                return agg;
            }, {});
            ...
            if (!_.isEmpty(couponData)) {
                ...
                if (payload.coupon_updates) {
                    for (const couponUpdate of payload.coupon_updates) {
                        ...
                        if (couponUpdate.old_id === clientLoyaltyCardId) {
                            client.loyalty_card_id = couponUpdate.id;
                        }
