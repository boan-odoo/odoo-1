...
const LinkPopoverWidget = Widget.extend({
    ...
    _onRemoveLinkClick(ev) {
        const lastElement = this.options.wysiwyg.lastElement;
        ev.preventDefault();
        this.options.wysiwyg.odooEditor.execCommand('unlink');
        ev.stopImmediatePropagation();
        if (isImg(lastElement)) {
            // Deselect and reselect so that options are updated.
            lastElement.parentElement.click();
            lastElement.click();
        }
    },
});
