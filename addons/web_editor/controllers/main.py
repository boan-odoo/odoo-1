...
from ..models.ir_attachment import SUPPORTED_IMAGE_EXTENSIONS, SUPPORTED_IMAGE_MIMETYPES, SUPPORTED_VIDEO_EXTENSIONS, SUPPORTED_VIDEO_MIMETYPES
...
class Web_Editor(http.Controller):
    ...
    def video_url_data(self, video_url, autoplay=False, loop=False,
                       ...
                       hide_dm_logo=False, hide_dm_share=False, to_save=False):
        ...
        return get_video_url_data(
            ...
            hide_dm_share=hide_dm_share, to_save=to_save
        )
        ...
        if filetype == "image":
            try:
                ...
                if mimetype not in SUPPORTED_IMAGE_MIMETYPES:
                    return {'error': _("Uploaded image's format is not supported. Try with: %s", ', '.join(SUPPORTED_IMAGE_EXTENSIONS))}
            except UserError:
            ...
            try:
                ...
                if len(data) > max_video_size * (1024 ** 2):
                    return {'error': _("Video size too big, uploaded videos must be smaller than %d Mb.", max_video_size)}
                if mimetype not in SUPPORTED_VIDEO_MIMETYPES:
                    return {'error': _("Uploaded video's format is not supported. Try with: %s. The uploaded video's mimetype is: %s",
                                       ', '.join(SUPPORTED_VIDEO_EXTENSIONS), mimetype)}
            except ValueError as e:
                return {'error': e.args[0]}
        elif filetype == "video_url":
            mimetype = "video/url"

        self._clean_context()
        attachment = self._attachment_create(
            name=name, data=data, res_id=res_id, res_model=res_model,
            thumbnail=kwargs.get("thumbnail", None), mimetype=mimetype,
    ...
    def add_video_thumbnail(self, attachment_id, thumbnail_data):
        video = request.env['ir.attachment'].browse(attachment_id)
