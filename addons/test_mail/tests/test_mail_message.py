...
class TestMessageValues(common.BaseFunctionalTest, common.MockEmails):
    ...
    def test_mail_message_values_fromto_long_name(self):
        ...
        done correctly (see task-2602862). """
        test_record = self.env['mail.test'].browse(self.alias_record.ids)
        test_record.write({'name': 'Super Long Name That People May Enter "Even with an internal quoting of stuff"'})
        msg = self.env['mail.message'].create({
            'model': test_record._name,
            'res_id': test_record.id
        })
        reply_to_name = test_record.name[:54] + '[...]'  # email is pigs@test.com = 13, with space + <..> + quotes + escaping -> 59 left
        reply_to_email = '%s@%s' % (test_record.alias_name, self.alias_domain)
        self.assertEqual(len(msg.reply_to), 78)
        self.assertEqual(msg.reply_to, formataddr((reply_to_name, reply_to_email)),
                         'Reply-To: strip out company_name and extra chars to stay under 78 chars')
        ...
        reply_to_name = test_record.name
        self.assertEqual(msg.reply_to, formataddr((reply_to_name, reply_to_email)),
                         'Reply-To: name should be complete without company if possible')
        ...
        self.assertEqual(msg.reply_to,
                         '%s@%s' % ('waaaay-too-long-alias-name-that-should-make-any-reply-to-blow-the-78-characters-limit', self.alias_domain),
                         'Reply-To: if email is too big, keep only the email')
