...
class HolidaysRequest(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if 'date_from' not in defaults:
            defaults['date_from'] = now
        if 'date_to' not in defaults:
            defaults['date_to'] = now

        if not self._context.get('leave_compute_date_from_to'):
            ...
            if employee_resource:
                employee_tz = timezone(employee_resource.tz or 'UTC')
            ...
            leave_date_from = leave_date_from and leave_date_from.astimezone(UTC).replace(tzinfo=None)
            leave_date_to = leave_date_to and leave_date_to.astimezone(UTC).replace(tzinfo=None)

            defaults['date_from'] = leave_date_from or defaults['date_from']
            defaults['date_to'] = leave_date_to or defaults['date_to']
            defaults = self._default_get_request_parameters(defaults)
        ...
        if self._context.get('leave_compute_date_from_to'):
            ...
            for values in vals_list:
                ...
                if values.get('date_from') and values.get('date_to') and employee_id:
                    employee_resource = resources.filtered(lambda res: res.employee_id.id == employee_id)
