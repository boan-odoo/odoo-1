...
const ProjectListController = ListController.extend({
    ...
    _countRecords(recurrenceIds, resIds) {
        return this._rpc({
            model: 'project.task',
            method: 'read_group',
            args: [
    ...
    async _stopRecurrence(recurringResIds, resIds, mode) {
        ...
        for (const record of this.getSelectedRecords()) {
            ...
            if (recurrenceId && !recurrenceIds.includes(recurrenceId)) {
                recurrenceIds.push(recurrenceId);
            }
        }
        // list recurrences that have tasks left after deleting/archiving
        let countsLeft = await this._countRecords(recurrenceIds, recurringResIds);
