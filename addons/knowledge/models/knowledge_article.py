...
class ArticleMembers(models.Model):
    ...
    def unlink(self):
        """ When removing a member, the constrain is not triggered.
        We need to check if the unlink does not lead to a validation error."""
        # TODO DBE: Check if the member to unlink is the last member with write access. If yes: raise ValidationError
        # Retrieve all articles that need at least one member with write permission
        articles = self.env['knowledge.article']
        for member in self:
            if member.article_permission != 'write':
                articles |= member.article_id

        # Unlink the members
        result = super(ArticleMembers, self).unlink()

        # Check if unlink led to a validation error
