...
class MicrosoftOutlookMixin(models.AbstractModel):
    ...
    def _compute_outlook_uri(self):
        ...
        if not microsoft_outlook_client_id or not microsoft_outlook_client_secret:
            self.microsoft_outlook_uri = False
            return
        ...
        for record in self:
            ...
            record.microsoft_outlook_uri = url_join(self._OUTLOOK_ENDPOINT, 'authorize?%s' % url_encode({
                ...
                'response_type': 'code',
                'redirect_uri': url_join(base_url, '/microsoft_outlook/callback'),
                'response_mode': 'query',
    ...
    def open_microsoft_outlook_uri(self):
        ...
        if not self.microsoft_outlook_uri:
        ...
        if not self.microsoft_outlook_access_token \
           ...
           or self.microsoft_outlook_access_token_expiration < now_timestamp:
            if not self.microsoft_outlook_refresh_token:
                raise UserError(_('Please connect your Outlook mail server.'))
            (
            ...
            _logger.info(
                'Microsoft Outlook: fetch new access token. Expire in %i minutes',
                (self.microsoft_outlook_access_token_expiration - now_timestamp) // 60)
        else:
            _logger.info(
                'Microsoft Outlook: reuse existing access token. Expire in %i minutes',
                (self.microsoft_outlook_access_token_expiration - now_timestamp) // 60)

        return 'user=%s\1auth=Bearer %s\1\1' % (user, self.microsoft_outlook_access_token)
