...
QUnit.test('rendering of visitor banner', async function (assert) {
    ...
    assert.strictEqual(
        document.querySelector('.o_VisitorBanner_avatar').getAttribute('src'),
    ...
    assert.strictEqual(
        document.querySelector('.o_VisitorBanner_country').getAttribute('src'),
...
QUnit.test('livechat with logged visitor should show visitor banner', async function (assert) {
    ...
    this.serverData.models['mail.channel'].records.push({
        ...
        livechat_operator_id: this.TEST_USER_IDS.currentPartnerId,
        livechat_visitor_id: 11,
        members: [this.TEST_USER_IDS.currentPartnerId, 12],
    });
    await this.start({
        discuss: {
            default_active_id: 'mail.channel_11',
