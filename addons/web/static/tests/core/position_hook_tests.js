...
QUnit.test("can add margin", async (assert) => {
    ...
    const popBox1 = container.querySelector("#popper").getBoundingClientRect();
    destroy(popper);

    TestComp = getTestComponent({ margin: 20 });
    popper = await mount(TestComp, container);
    const popBox2 = container.querySelector("#popper").getBoundingClientRect();
    destroy(popper);
...
QUnit.test("can place the popper t-ref on any node", async (assert) => {
    ...
    const TestComp = getTestComponent({
        onPositioned: (el) => {
            assert.notOk(container.querySelector("#not-popper") === el);
            assert.ok(container.querySelector("#popper") === el);
        },
    });
