...
class TestMailTemplate(MailCommon):
    ...
    def test_mail_template_reset(self):
        ...
        mail_template = self.env.ref('mail.email_template_test').with_context(lang=self.env.user.lang)
        self.assertFalse(mail_template.is_body_updated, "Mail Template Body should not be updated")

        mail_template.write({'body_html': '<div>Hello</div>'})
        self.assertTrue(mail_template.is_body_updated, "Mail Template Body should be updated")

        mail_template.reset_mail_template()
