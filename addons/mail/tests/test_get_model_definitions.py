...
@odoo.tests.tagged('-at_install', 'post_install', 'this')
class TestGetModelDefinitions(HttpCase):
    ...
    def test_access_all_model_fields(self):
        ...
        self.assertTrue(
            all(fname in model_definitions['res.users'].keys()
                for fname in ['email', 'name', 'partner_id']))
        self.assertTrue(
            all(fname in model_definitions['res.partner'].keys()
                for fname in ['active', 'date', 'name']))
