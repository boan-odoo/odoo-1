...
class AccountEdiFormat(models.Model):
    ...
    def _get_edi_peppol_info(self, company, customer=None):
        ...
        if not company.country_id:
            #_logger.info(f"Unable to use Peppol: No country set for company: {company.name}")
            return
        if customer is not None and not self._is_edi_peppol_customer_valid(customer):
            #_logger.info(f"Unable to use Peppol: Customer: {customer.name}  not in COUNTRY_EAS")
            return
    ...
    def _post_invoice_edi(self, invoices, test_mode=False):
        ...
        # e.g. if we send an invoice to a american company, peppol_info is None, no attachment is constructed
